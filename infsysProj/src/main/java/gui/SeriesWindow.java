...
public class SeriesWindow extends JFrame {
	...
	public static enum ItemsPerPage {
		TWENTY("10 items per page", 10, 0), FIFTY("50 items per page", 50, 1), HUNDRED("100 items per page", 100, 2), TWOHUNDRED("200 items per page", 200, 3), FIVEHUNDRED("500 items per page", 500, 4), THOUSAND("1000 items per page", 1000, 5), ALL("All items", Integer.MAX_VALUE, 6);
	...
	public SeriesWindow() {
		...
		numberOfItemsLabel = new JLabel("1-50 of " + String.valueOf(allSeries.size()) + " items");
		c.fill = GridBagConstraints.HORIZONTAL;
		c.weightx = 1;
		c.weighty = 0;
		c.gridwidth = 1;
		...
		table.getTableHeader().addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				case 0:
					nNameClicked++;
					if (nNameClicked % 2 == 0) {
						Collections.sort(allSeries, new Comparator<Series>() {
							@Override
	...
	public void reloadTable() {
		...
		if (itemsPerPageIndex == ItemsPerPage.ALL) {
			...
			numberOfItemsLabel.setText(String.valueOf(pageNumber * itemsPerPageIndex.getNumber() + 1) + " - " + String.valueOf((int) Math.min(size, (pageNumber + 1) * itemsPerPageIndex.getNumber())) + " of " + String.valueOf(allSeries.size()) + " items");
			pageTextField.setText(String.valueOf(pageNumber + 1));
		}
		currentSeries = allSeries.subList(itemsPerPageIndex.getNumber() * pageNumber, Math.min(itemsPerPageIndex.getNumber() * (pageNumber + 1), size));
		tableModel.changeData(currentSeries);

	}

	private int numberOfPages() {
		if (allSeries.size() < itemsPerPageIndex.getNumber()) {
			return 1;
		} else {
			return (int) Math.floor(allSeries.size() / itemsPerPageIndex.getNumber()) + 1;
		}
	}

	public SeriesWindow getMe() {
		return this;
	}
